...
/**
* A sequence of qs_type instances.
*/
public final class qs_typeSeq extends LoanableSequence implements Copyable {
    // -----------------------------------------------------------------------
    // Package Fields
    // -----------------------------------------------------------------------
    ...
    */
    ...
    public qs_typeSeq (int initialMaximum) {
        super(qs_type.class, initialMaximum);
    }

    public qs_typeSeq (Collection elements) {
        super(qs_type.class, elements);
    }

    public  qs_type get(int index) { 
        return (qs_type) super.get(index); 
    }
        ...
        for(int i = 0; (i < origSize) && (i < srcSize); i++){
            if (typedSrc.get(i) == null){
                ...
                if (get(i) == null){ 
                    ...
                    set(i, qs_type.create());
                }
                set(i, ((Copyable) get(i)).copy_from(typedSrc.get(i)));
            }
        }

        // copy 'new' qs_type objects (beyond the original size of this object)
